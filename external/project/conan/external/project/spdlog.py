...
class SpdlogConan(ConanFile):
    ...
    def requirements(self):
        if not self.options.get_safe("use_std_fmt"):
            fmt_version = self.conan_data["spdlog"]["fmt_version_mapping"][self.version]
    ...
    def source(self):
        get(self, **self.conan_data["spdlog"][self.version], strip_root=True)
    ...
    def generate(self):
        if not self.options.header_only:
            ...
            tc.variables["SPDLOG_DISABLE_DEFAULT_LOGGER"] = True
