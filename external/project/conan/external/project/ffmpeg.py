...
class FFMpegConan(ConanFile):
    ...
    def config_options(self):
        os_distro = self.conf.get("user.os:distro", "")
        ...
        if self.settings.os != "Linux" or os_distro != "Xrlinux":
            del self.options.with_libmp3lame
    ...
    def requirements(self):
        ...
        if self.options.get_safe("with_libmp3lame"):
    ...
    def source(self):
        get(self, **self.conan_data["ffmpeg"][self.version],
    ...
    def _target_os(self):
        if self.settings.os == "Windows":
            return "mingw32" if self.settings.compiler == "gcc" or self.settings.compiler == "clang" else "win32"
    ...
    def _create_toolchain(self, target_arch):
        tc = AutotoolsToolchain(self, None if target_arch != "x86_64" else target_arch)
    ...
    def generate_arch(self, target_arch):
        ...
        tc = self._create_toolchain(target_arch)
        ...
        args = [
            ...
            opt_enable_disable("libmp3lame", self.options.get_safe("with_libmp3lame")),
        ...
        if target_arch != "x86_64":
            ...
            args.append(f"--arch={target_arch}")
        ...
        if cc:
            if target_arch != "x86_64":
                ...
                args.append(f"--cc={unix_path(self, cc)} -arch x86_64")
                args.append(opt_enable_disable("cross-compile", True))
        ...
        if ld:
            if self.settings.os != "Android" and self.settings.os != "Windows" and self.settings.os != "Linux":
                # Android and Windows use clang instead of ld
        ...
        if self.settings.os == "Windows" and not is_msvc(self):
            if cc:
                ...
                m = re.match(r"(.*)-(gcc|clang)$", basename)
                if m:
                    args.append(f"--cross-prefix={m.group(1)}-")
            #args.append("--cross-prefix=x86_64-w64-mingw32-")
    ...
    def merge_libraries(self, dir1, dir2, output_dir):
        def is_binary_file(file_path):
            try:
                result = subprocess.run(['lipo', '-info', file_path], capture_output=True, text=True)
                return "Non-fat file" in result.stdout or "Architectures in the fat file" in result.stdout
            except subprocess.CalledProcessError:
                return False
        for root, _, files in os.walk(dir1):
            for file in files:
                lib1 = os.path.join(root, file)
                lib2 = lib1.replace(dir1, dir2)

                if os.path.exists(lib2) and is_binary_file(lib1) and is_binary_file(lib2):
                    # 创建输出目录的结构
                    ...
                    # 合并库文件
                    output_lib = os.path.join(output_path, file)
                    try:
                        self.run(f"lipo -create {lib1} {lib2} -output {output_lib}")
                        shutil.copy2(output_lib, lib1)
                    except Exception as e:
                        self.output.info(f"error({e}): lipo -create {lib1} {lib2} -output {output_lib}")
    ...
    def build(self):
        ...
        if self.info.settings.os == "Macos":
            x86_64_build_dir = os.path.join(self.build_folder, "x86_64")
            os.makedirs(x86_64_build_dir, exist_ok=True)
            with chdir(self, x86_64_build_dir):
                x86_64_autotools = Autotools(self, "x86_64")
                x86_64_autotools.configure()
                x86_64_autotools.make()

            # 创建universal目录
            universal_dir = os.path.join(self.build_folder, "universal")
            os.makedirs(universal_dir, exist_ok=True)
            # 自动合并同级目录相同文件名的库
            self.merge_libraries(self.build_folder, x86_64_build_dir, universal_dir)
