...
class PerfettoConan(ConanFile):
    ...
    def export_sources(self):
        copy(self, "perfetto.cmake", src=os.path.join(self.recipe_folder, "cmake"), dst=self.export_sources_folder)
        rename(self, src=os.path.join(self.export_sources_folder, "perfetto.cmake"), dst=os.path.join(self.export_sources_folder, "CMakeLists.txt"))

    def config_options(self):
        if Version(self.version).pre == 'shared':
    ...
    def source(self):
        get(self, **self.conan_data["perfetto"][self.version], strip_root=True)
    ...
    def build(self):
        if self.settings.os == "Windows":
            replace_in_file(self, os.path.join(self.source_folder, "sdk", "perfetto.cc"),
            "using pid_t = int;",
            "",)
        cmake = CMake(self)
