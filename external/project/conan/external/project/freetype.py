...
class FreetypeConan(ConanFile):
    ...
    def source(self):
        get(self, **self.conan_data["freetype"][self.version], strip_root=True)
    ...
    def _patch_sources(self):
        ...
        # # Add 32-bit fcntl definitions for arm64 linux
        # os_distro = self.conf.get("user.os:distro", "")
        # if self.settings.os == "Linux" and os_distro == "Xrlinux":
        #     ftconfig_path = os.path.join(self.source_folder, "builds", "unix", "ftconfig.h.in")
        #     if os.path.exists(ftconfig_path):
        #         replace_in_file(
        #             self,
        #             ftconfig_path,
        #             "#undef HAVE_UNISTD_H",
        #             "#undef HAVE_UNISTD_H\n\n"
        #             "/* Force to use 32-bit fcntl on arm64 linux(xrlinux)*/\n"
