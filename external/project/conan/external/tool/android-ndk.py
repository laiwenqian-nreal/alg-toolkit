...
class AndroidNDKConan(ConanFile):
    ...
    def _settings_os_supported(self):
        return self.conan_data["android-ndk"][self.version].get(str(self.settings.os)) is not None

    @property
    def _settings_arch_supported(self):
        return self.conan_data["android-ndk"][self.version].get(str(self.settings.os), {}).get(str(self._arch)) is not None
    ...
    def build(self):
        ...
        if major >= 23:
            data = self.conan_data["android-ndk"][self.version][str(self.settings.os)][str(self._arch)]
            self._unzip_fix_symlinks(url=data["url"], target_folder=self.source_folder, sha256=data["sha256"])
        else:
            get(self, **self.conan_data["android-ndk"][self.version][str(self.settings.os)][str(self._arch)],
